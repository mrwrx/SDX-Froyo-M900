...
#if (CONFIG_INFOBOWLQ_REV >= CONFIG_INFOBOWLQ_REV00)
...
static struct platform_device *infobowlq_devices_07[] __initdata = {
#if defined(CONFIG_S3C_DMA_PL080_SOL)
	&s3c_device_dma0,
	&s3c_device_dma1,
	&s3c_device_dma2,
...
#if defined(CONFIG_MACH_MAX)
	platform_add_devices(infobowlq_devices_07, ARRAY_SIZE(infobowlq_devices_07));
#elif defined (CONFIG_MACH_VITAL)
	platform_add_devices(infobowlq_devices, ARRAY_SIZE(infobowlq_devices));
#else
#if defined(CONFIG_MACH_VINSQ)
	//i2c_register_board_info(2, i2c_devs2, ARRAY_SIZE(i2c_devs2));
	if(system_rev >= CONFIG_INFOBOWLQ_REV07)
	{
		platform_add_devices(infobowlq_devices_07, ARRAY_SIZE(infobowlq_devices_07));
	}
	else
...
#if defined(CONFIG_MACH_VINSQ)
MACHINE_START(INFOBOWLQ, "SPH-M910")
#elif defined(CONFIG_MACH_VITAL)
MACHINE_START(INFOBOWLQ, "SPH-M920")
#elif defined(CONFIG_MACH_QUATTRO)
MACHINE_START(INFOBOWLQ, "SWD-S500")
#elif defined(CONFIG_MACH_MAX)
MACHINE_START(INFOBOWLQ, "SCH-I100")
#else
MACHINE_START(INFOBOWLQ, "SPH-M900")
#endif
...
void s3c_config_wakeup_source(void)
{
    ...
    if(system_rev<CONFIG_INFOBOWLQ_REV14)
    {
